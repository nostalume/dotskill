...
Record row grain, keys, schema and order, temporal semantics, missing-data policy,
artifacts, random seeds, numerical tolerances, runtime, peak memory, plots, and
side effects on representative data.
...
- Preserve typed provenance when it authorizes persistence. Do not hide source
  coverage repair or a changed missing-value policy as cleanup.
...
Completion requires rows, schema, numerical results, artifacts, and effects to
match within stated tolerances; runtime and memory are measured; obsolete wrappers,
serializers, scripts, and presentation backends have zero references.
